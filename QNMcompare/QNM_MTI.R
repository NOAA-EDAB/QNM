...
if(Sys.info()['sysname']=="Linux"){
    qnm.dir    <- "/home/slucey/slucey/QNM/QNMcompare"
    qpress.dir <- file.path(qnm.dir, "Qpress_results")
    out.dir    <- file.path(qnm.dir, "Outputs")
}
...
load(file.path(qnm.dir, 'WSS28.RData'))

#Run Ecopath
GOM <- rpath(GOM.28b, "Gulf of Maine")
WSS <- rpath(WSS28.params, "Western Scotian Shelf")

#Run MTI
gom.mti <- MTI(GOM, GOM.28b)
gom.mti.down <- MTI(GOM, GOM.28b, increase = F)

wss.mti <- MTI(WSS, WSS28.params)
wss.mti.down <- MTI(WSS, WSS28.params, increase = F)
...
#Plot MTI results
n.pred <- ncol(wss.mti)
labs <- WSS28.params$model$Group

opar <- par()

for(igroup in 1:n.pred){
    if(igroup %in% seq(1, n.pred, 4)){
        png(file = file.path(out.dir, paste0('WSS_MTI_Groups_', igroup, '_',
                                             igroup + 3, '.png')),
            height = 1500, width = 2000, res = 200)
        par(mfcol = c(4, 1), mar = c(0,0,0,0), oma = c(14, 14, 0, 0))
    }
    bars <- barplot(wss.mti[, igroup], ylim = c(-1, 1), axes = F)
    mtext(2, text = labs[igroup], las = T, line = -2, cex = 1)
    #Add pos/neg colors
    pos.value <- wss.mti[, igroup]
    neg.value <- wss.mti[, igroup]
    pos.value[which(pos.value < 0)] <- 0
    neg.value[which(neg.value > 0)] <- 0
    barplot(pos.value, ylim = c(-1, 1), add = T, col = 'blue', axes = F)
    barplot(neg.value, ylim = c(-1, 1), add = T, col = 'red', axes = F)
...
scenarios <- c('Phytoplankton_up',
               ...
               'Fishery_down')
decrease <- c(F, T, F, T, rep(F, 4), T, F, F, T)
focal.group <- c(25, 14, 22, 21, 13, 11, 10, 7, 7, 3, 28, 28)

out.order <- WSS28.params$model$Group
for(iscene in 1:12){
    if(decrease[iscene]){
        scene.out <- data.table(Group = out.order, 
                                mti = wss.mti.down[, focal.group[iscene]])
    } else {
        scene.out <- data.table(Group = out.order, mti = wss.mti[, focal.group[iscene]])
    }
    
    #Load QPress results
    for(imod in 1:5){
        mod.name <- paste0('WSS28results', imod, scenarios[iscene], '.csv')
        qpress.results <- as.data.table(read.csv(file.path(qpress.dir, mod.name)))
        setnames(qpress.results, c('X', 'X.', 'X0', 'X..1'), c('Group', 'Neg', 'Neu', 'Pos'))
    ...
    #Plot output
    png(file = file.path(out.dir, paste0('WSS_', scenarios[iscene], '.png')), 
        height = 1500, width = 2000, res = 200) 
    par(mar = c(10, 6, 4, 2))
    bars <- barplot(as.matrix(t(scene.out)), beside = T, 
                    col = c('#7fc97f','#beaed4','#fdc086','#ffff99',
