...
groups <- c('Whales', 'Toothed cetaceans', 'Seals', 'Sea birds', 'Sharks', 
            ...
            'Squids', 'Megabenthos', 'Small crab other arthropoda', 'Shrimps', 'Scallop', 
...
pb <- c(0.071,0.18,0.147007,0.25,0.18,0.4,0.5341109,0.3798308,0.2320817,0.6959251,0.7432774,4,
        0.8221899,2.759435,3,0.342,0.69,0.75,1.25,1.32583,15.51,3.04,29.2, 
        82.8, 104.938, 70.639, rep(NA, 3))

qb <- c(4.94,14.5,7.338632,87.6,4.78,4.24,2.868397,3.254083,2.447539,3.475426,3.532147,11.33333,
        5.481265,18.39623 ,rep(NA, 5), 8.83887,62.05, 19.5, 73, rep(NA, 6))
...
WSS28.params$model[Group %in% c('megabenthos', 'Small crab- other arthropoda', 'Shrimps', 'Scallop', 
                                'Bivalves', 'Other molluscs', 'Worms', 'Meiofauna'),
                 ProdCons := 0.15]
...
ba <- c(0.0032566,0,0.003702101,0,-0.001951868,0,-0.05736,0.044246, rep(0, 4), 0.0301295, 0,0,
        0.03234285,rep(0,12), NA)
# ba<-c(rep(0,28), NA)
WSS28.params$model[, BioAcc  := ba]
WSS28.params$model[, Unassim := c(rep(0.2, 21), rep(0.4, 3), 0.2, rep(0, 3), NA)]

#Detrital fate
WSS28.params$model[, Detritus := c(rep(1, 26), rep(0, 3))]
WSS28.params$model[, Discards := c(rep(0, 28), 1)]
...
disc <- c(0,0.002995,0,2.4E-05,
          ...
          0.008776,
          0.000633,
          8E-06 ,0,0,4.1E-05,0,0.019869, rep(0, 8), NA)
WSS28.params$model[, Fleet1      := land]
WSS28.params$model[, Fleet1.disc := disc]
...
# save(DC.data, file='DC.data.RData')
load('DC.data.RData')
#-------------------------------------------
...
setnames(WSS28.params$diet, paste0('V', 1:26), DC.groups)
...
# qpress.WSS28 <- t(qpress.pred[, .SD, .SDcols = c(M2.cols, 'M2.Detritus', 'M2.Discards')]) + 
